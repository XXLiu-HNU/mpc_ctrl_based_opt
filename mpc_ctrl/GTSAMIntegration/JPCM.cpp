...
{
  // IMU noise
  auto imu_factor_noise = noiseModel::Diagonal::Sigmas((Vector(9) << Vector3::Constant(param_.factor_graph.acc_sigma_x * dt * dt * 0.5f + param_.factor_graph.acc_sigma_x * dt * dt),
                                                        Vector3::Constant(param_.factor_graph.gyro_sigma_x * dt), Vector3::Constant(param_.factor_graph.acc_sigma_x * dt))
                                                           .finished());
...
{
  // 定义控制输入平滑性约束的噪声模型（用于相邻控制输入的 “jerk” 约束）,物理意义：限制相邻控制输入的变化率（jerk），避免控制量突变，保证飞行平稳
  // Vector4对应控制输入
  // u = [ T, ω x, ωy, ωz]^T（推力T和三个轴的角速度），噪声标准差越大，约束越弱。
  ...
  // 定义动力学约束的噪声模型（对应之前的DynamicsFactorTGyro）
  // 9 维向量对应动力学因子的 3 维位置误差、3 维旋转误差、3 维速度误差（与DynamicsFactorTGyro的err向量维度一致）
  // 标准差（如DYNAMIC_P_COV为位置误差的标准差）决定了动力学模型的置信度：标准差越小，动力学约束越强（越信任模型预测）
  auto dynamics_noise = noiseModel::Diagonal::Sigmas((Vector(9) << Vector3::Constant(param_.factor_graph.DYNAMIC_P_COV),
                                                      Vector3::Constant(param_.factor_graph.DYNAMIC_R_COV), Vector3::Constant(param_.factor_graph.DYNAMIC_V_COV))
                                                         .finished());
  // 定义速度参考轨迹约束的噪声模型，用于将估计速度约束到期望速度,3 维对应 x/y/z 方向的速度，标准差CONTROL_V_COV越大，对期望速度的约束越弱
  ...
  // 定义控制输入极限约束的噪声模型，用于限制控制输入（推力、角速度）在安全范围内，4 维对应控制输入u的 4 个分量，这里标准差设为 1.0，是约束强度的基准
  ...
  _graph = graph_positioning_;
  // 定义控制输入的优化范围：从begin_u=0到end_u=opt_traj_lens_（优化轨迹的长度）
  uint16_t begin_u = 0;
  uint16_t end_u = opt_traj_lens_;

  uint16_t latest_state_idx = 0;
  // 根据state_idx判断是否为首次优化，设置滑动窗口中最新的状态索引（latest_state_idx），作为动力学约束的起点
  if (state_idx == 1)
  {
  ...
  for (uint16_t idx = begin_u; idx < end_u; idx++)
  {
    ...
    // 为控制输入U(idx)设置初始值init_input（推力 10N，角速度为 0），首次添加时插入，否则更新
    ...
    // 定义姿态参考约束的噪声标准差（3 维对应三个旋转轴），用于将估计姿态约束到期望姿态
    ...
    // 为每个状态添加先验因子（PriorFactor），将状态（位姿和速度）约束到期望轨迹des_seq
    if (idx == end_u - 1)
    {
...
{
  // 定义输入加加速度（Jerk）的噪声模型，用于约束连续控制输入之间的平滑性（避免突变）
  auto input_jerk = noiseModel::Diagonal::Sigmas(Vector4(param_.factor_graph.INPUT_JERK_T,
                                                         param_.factor_graph.INPUT_JERK_M, param_.factor_graph.INPUT_JERK_M, param_.factor_graph.INPUT_JERK_M3));
  // 定义动力学模型的噪声模型，用于描述状态转移（从当前到下一时刻）的不确定性
  auto dynamics_noise = noiseModel::Diagonal::Sigmas((Vector(9) << Vector3::Constant(param_.factor_graph.DYNAMIC_P_COV),
                                                      Vector3::Constant(param_.factor_graph.DYNAMIC_R_COV), Vector3::Constant(param_.factor_graph.DYNAMIC_V_COV))
                                                         .finished());

  // 定义初始状态的噪声模型（来自 Vicon 等传感器的测量噪声）
  ...
  // 定义参考轨迹速度的噪声模型，用于约束优化轨迹的速度与期望速度（des_seq）的偏差
  ...
  auto clf_sigma = noiseModel::Diagonal::Sigmas(Vector4(1.0, 1.0, 1.0, 1.0));
  // 自定义因子，用于约束控制输入（推力和陀螺力矩）的物理极限（如最大 / 最小推力、最大 / 最小角速度）
  ControlLimitTGyroFactor cntrolLimitTGyroFactor(U(0), clf_sigma, param_.factor_graph.low, param_.factor_graph.high,
                                                 param_.factor_graph.glow, param_.factor_graph.ghigh, param_.factor_graph.thr, param_.factor_graph.gthr, param_.factor_graph.alpha);
  graph.add(cntrolLimitTGyroFactor);
  // 定义阻力系数向量，用于动力学模型中对空气阻力的建模（符号为负，与速度方向相反）
  gtsam::Vector3 drag_k(-param_.rt_drag.x, -param_.rt_drag.y, -param_.rt_drag.z);

  for (uint16_t idx = 0; idx < param_.factor_graph.OPT_LENS_TRAJ; idx++)
  {
    // 自定义动力学因子，约束相邻时刻的状态与控制输入的关系（基于运动学 / 动力学方程）
    // 参数：dt（时间间隔）、param_.mass（质量）、drag_k（阻力系数）、dynamics_noise（噪声模型）
    DynamicsFactorTGyro dynamics_factor(X(idx), V(idx), U(idx), X(idx + 1), V(idx + 1), dt, param_.mass, drag_k, dynamics_noise);
    graph.add(dynamics_factor);
    // 从期望状态序列des_seq（输入参数）中提取第idx时刻的位姿（q为四元数，p为位置）和速度（v），作为优化中X(idx+1)（位姿）和V(idx+1)（速度）的初始猜测值
    gtsam::Pose3 pose_idx(gtsam::Rot3(des_seq[idx].q), des_seq[idx].p);
    gtsam::Vector3 vel_idx = des_seq[idx].v;
