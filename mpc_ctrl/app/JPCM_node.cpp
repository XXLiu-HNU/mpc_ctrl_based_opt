...
int main(int argc, char *argv[])
{
    ...
    nh.param<std::string>("target_topic", target_topic, "/mavros/setpoint_raw/local");
    nh.param<std::string>("imu_topic", imu_topic, "/mavros/imu/data");
    nh.param<std::string>("attitude_target_topic", attitude_target_topic, "/mavros/setpoint_raw/attitude");
    ...
    ros::Subscriber cmd_sub =
        nh.subscribe<mpc_ctrl::PositionCommand>(target_topic,
                                                100,
    ...
    ros::Subscriber imu_sub =
        nh.subscribe<sensor_msgs::Imu>(imu_topic, // Note: do NOT change it to /mavros/imu/data_raw !!!
                                       100,
    ...
    fsm.ctrl_FCU_pub = nh.advertise<mavros_msgs::AttitudeTarget>(attitude_target_topic, 10);
