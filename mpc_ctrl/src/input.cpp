...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    if (last_mode < API_MODE_THRESHOLD_VALUE && mode > API_MODE_THRESHOLD_VALUE)
        ...
        enter_hover_mode = false;
    // 当前是否处于悬停模式。当 mode > API_MODE_THRESHOLD_VALUE（0.75） 时为 true，表示系统进入自动悬停状态
    if (mode > API_MODE_THRESHOLD_VALUE)
        is_hover_mode = true;
    else
        is_hover_mode = false;

    // 在自动悬停模式下，控制无人机是否允许接收外部指令（如自主导航、轨迹跟踪命令）
    if (is_hover_mode)
    {
...
Command_Data_t::Command_Data_t()
{
    rcv_stamp = ros::Time(0);
    recv_new_msg = false;
    first_recv_new_msg = false;
}

void Command_Data_t::feed(mpc_ctrl::PositionCommandConstPtr pMsg)
{
    msg = *pMsg;
    rcv_stamp = ros::Time::now();
